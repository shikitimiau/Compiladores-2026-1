...
expr_to_AFNEp regex = fst (expr_to_AFNEp_aux regex 0)


-- ------------------------------------------------------------------------------
-- Función auxiliar para realizar el autómata finito no determinista con
-- transiciones epsilon de una regex válida.
-- Se utiliza un contador global para generar nombres únicos para los estados.
-- ------------------------------------------------------------------------------
expr_to_AFNEp_aux :: Regex -> Int -> (AFNEp, Int)
-- Caso base: AFNEp para un símbolo terminal (caracter).
expr_to_AFNEp_aux (Symbol a) n =
  (AFNEp { 
    -- Agregamos 2 estados nuevos
    estados = [q0, q1], 
    -- El símbolo recibido se considera su actual alfabeto.
    alfabeto = [a], 
    -- Agregamos la nueva transicion de q0 a q1 con el simbolo especificado
    transiciones = [(q0, Just a, [q1])], 
    -- Especificamos quien es el estado inicial y quien es el final 
    inicial = q0,
    final = q1
  }, n + 2) -- Contamos 2 estados más que fueron añadidos en este paso
  where
  -- Nombramos los estados nuevos
  q0 = "q" ++ show n
  q1 = "q" ++ show (n + 1)
...
-- Utilizamos la fromList para eliminar duplicados y toList para que se convierta
-- nuevamente en una lista.
-- ------------------------------------------------------------------------------
rmDup :: (Ord a) => [a] -> [a]
rmDup xs = Set.toList (Set.fromList xs)